...
func (self *RefreshHelper) refreshCommitsWithLimit() error {
	...
	commits, err := self.c.Git().Loaders.CommitLoader.GetCommits(
		git_commands.GetCommitsOptions{
			...
			FilterGrep:           self.c.Modes().Filtering.GetGrep(),
...
func (self *RefreshHelper) refreshReflogCommits() error {
	...
	refresh := func(stateCommits *[]*models.Commit, filterPath string, filterAuthor string, filterGrep string) error {
		commits, onlyObtainedNewReflogCommits, err := self.c.Git().Loaders.ReflogCommitLoader.
			GetReflogCommits(lastReflogCommit, filterPath, filterAuthor, filterGrep)
		if err != nil {
			return err
		}
	...
	if self.c.Modes().Filtering.Active() {
		if err := refresh(&model.FilteredReflogCommits, self.c.Modes().Filtering.GetPath(), self.c.Modes().Filtering.GetAuthor(), self.c.Modes().Filtering.GetGrep()); err != nil {
			return err
		}
	} else {
